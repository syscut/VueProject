    ...
    <v-col cols="12">
      ...
      >
        ...
        <template v-slot:item.qt_desc="{ value }">
          <template v-if="isImg(value)">
            <label @click="showImg" class="cursor blue--text"
              >點擊顯示圖片(點擊圖片隱藏)</label
            >
            <v-img
              class="cursor"
        ...
        <!-- 插入表頭與搜尋欄 ----------------------------------------------->
        <template v-slot:top="{ originalItemsLength, pagination }">
          <!-- top:插入表頭 -->
          <v-toolbar flat>
            <!-- falt:去除陰影 -->
            <v-toolbar-title>
                      ...
                      <v-row>
                        ...
                        <v-col cols="4">
                          ...
                          <v-select
                            ...
                            v-show="
                              editMode == 'new_item' || editMode == 'answer'
                            "
                            :items="ans_types"
                            :label="
                              editMode == 'new_item' ? '題目類型' : '答案類型'
                            "
                            prepend-icon="mdi-alpha-a-circle"
                            @change="showFileUpload = !showFileUpload"
                          ></v-select>
                        </v-col>
                        <v-col cols="12" v-show="!showFileUpload">
                          <v-textarea
                            v-model="editedItem.qt_desc"
                            prepend-icon="mdi-note-text-outline"
                            :rules="max255chars"
                            @keyup="
                              editedItem.qt_desc = check(editedItem.qt_desc)
                            "
                        ...
                        <v-col
                          cols="12"
                          v-show="showFileUpload && editMode == 'edit_item'"
                        >
                          <v-img
                            max-width="400"
                        ...
                        >
                          ...
                          >
                            <p>檔案名稱：{{ n.fileName }}</p>
                            <v-img max-width="450" :src="n.fileSrc"></v-img>
                          </v-col>
        ...
        <template v-slot:body.append>
          <tr>
            ...
            <td colspan="2" class="px-0"></td>

            <td></td>
            <td class="px-0">
              <v-text-field v-model="rec" dense> </v-text-field>
            </td>
            <td class="px-0">
              <v-select dense v-model="selectedRecIcon" :items="filterRecIcon">
      ...
      <v-snackbar v-model="snack" :timeout="1000" :color="snackColor">
        ...
        <template v-slot:action="{ attrs }">
          <v-btn v-bind="attrs" text @click="snack = false"> 關閉提示 </v-btn>
        </template>
      </v-snackbar>
    </v-col>
  </v-row>
...
export default {
  ...
  computed: {
      ...
      return [
        ...
        },
        {
          ...
          filter: (v) =>
            !this.search
              ? true
              : !this.isImg(v) && v.toString().indexOf(this.search) !== -1,
        },
        {
          text: "分數",
          value: "qt_rec",
          filter: (value) => {
            ...
            return this.selectedRecIcon == "<="
              ? value <= parseInt(this.rec)
              : this.selectedRecIcon == ">="
              ? value >= parseInt(this.rec)
              : value == parseInt(this.rec);
          },
          width: "45px",
    ...
    contents: {
      get() {
        try {
          return this.content.map((val) => ({
            ...val,
            isSelectable: val.is_item == "false" && val.end_mark != "x",
          }));
        } catch (error) {
    ...
    formTitle() {
      return this.editMode === "new"
        ...
        : this.editMode == "edit"
        ? ["編輯作業", "mdi-file-edit"]
        : this.editMode == "edit_item"
        ? ["編輯項次", "mdi-file-edit"]
        : this.editMode == "new_item"
        ? ["新增項次", "mdi-playlist-plus"]
        : ["新增答案", "mdi-new-box"];
    },
    footerProps() {
      return {
  ...
  methods: {
    ...
    testClick() {
      let t =
        '{ "�����(1).png": "http://gfcweb/cgi-bin/prgm360w_dl_handle.pl?id=6079&name=�����(1).png ", "����.png": "http://gfcweb/cgi-bin/prgm360w_dl_handle.pl?id=6080&name=����.png", "�s��r���.txt": "http://gfcweb/cgi-bin/prgm360w_dl_handle.pl?id=6081&name=�s��r���.txt" }';
        ...
        reader.onload = (e) => {
          ...
          image.onload = (i) => {
            this.filesContent.push({
              fileName: v.name,
              fileSrc: URL.createObjectURL(v),
              fileWidth: i.target.width,
              fileHeight: i.target.height,
            });
          };
        };
      });
    ...
    getFileSrcSingle() {
      ...
      Object.assign(this.editedItem, {
        qt_desc: URL.createObjectURL(this.selectedFile),
      });
      this.fileLoading = false;
    },
    showImg(el) {
    ...
    checkIndex(item) {
      ...
      this.editedIndex = this.content.findIndex(
        (cont) => cont.row_id == item.row_id
      );
    },
    nweRow() {
    ...
    removeItem(item) {
      ...
      this.deleteTitle =
        this.editedItem.end_mark == "x"
          ? {
              title: "取消作廢",
              content: "確認取消？",
              btnText: "確認取消作廢",
            }
          : { title: "作廢作業", content: "確認作廢？", btnText: "確認作廢" };
      this.dialogDelete = true;
    },
    ...
    close() {
      //關掉新增視窗，編輯模式要回復輸入框為預設(editedIndex !== -1)，新增模式則保留(editedIndex === -1)
      this.dialog = false;
      this.selected = [];
      this.$nextTick(() => {
    ...
    async save() {
      ...
      if (this.editMode == "new_item" && this.showFileUpload == false) {
        let item_index = this.content
          .map((v) => v.is_item)
          .indexOf("false", this.editedIndex + 1);

        await axios
          .get(this.api + this.searchParam(), {
            params: {
              aq_type: "Q",
            },
          })
      ...
      if (this.showFileUpload == true && this.editMode == "edit_item") {
        ...
        this.valid = false;
        let parseS110 =
          this.selectedS110 == "測試"
            ? "test"
            : this.selectedS110 == "管理者"
            ? "adm"
            : this.selectedS110;
        ...
        await axios
          .post("http://gfcweb/cgi-bin/prgm360w_up_handle.pl", formData)
          .then((d) => {
            console.log(d.data);
            Object.assign(this.content[this.editedIndex], {
              qt_desc: d.data.ans_desc,
            });
      ...
      if (this.showFileUpload == true && this.editMode == "new_item") {
        ...
        let test = await en.uploadFiiles(
          this.selectedFile,
          "docd400w",
          "03/25/2022"
        );
    ...
    async updateRecFun() {
      ...
      await axios
        ...
        .then((d) => {
          ...
          this.content
            .filter((v) => selectId.includes(v.row_id))
            .forEach((val) => (val.qt_rec = this.updateRec));
