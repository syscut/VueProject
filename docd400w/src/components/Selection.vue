<template>
    ...
    <v-select
      ...
      no-data-text="請先選擇機種"
      clearable
      @change="selectedQtType==null? selectedQtType='':selectedQtType"
    ></v-select>
    </v-col>
...
export default {
  ...
  methods:{
    getQtType(){
      ...
      for(item of type){
        obj = {};
         type = (item == 'F') ? '填充題': (item == 'Q') ? '問答題':(item == 'O') ? '是非題':'選擇題';
         obj['text'] = type;
         obj['value'] = item;
    ...
    getDesc(){
      ...
      axios.get('http://gfcweb/xml/xml/kim_test_api.html?t=desc&s110='+gfc.encodeBig5(this.selectedS110)+'&qt_type='+this.selectedQtType).then(data=>{
        this.searchBtnDisable = false;
        this.loadTable = '';
  ...
  mounted(){
      ...
      this.loadingMsg = '讀取中...'
      axios.get('http://gfcweb/xml/xml/kim_test_api.html',{
        params:{
          t:'get_s110'
          }}).then(data=>{
